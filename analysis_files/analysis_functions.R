...
f_plot_compare_experts <- function (lim_1, lim_2, quantity, units, densities, que_no) {
  ...
  temp_expert_ids <- colnames(densities)[-ncol(densities)]
  temp_expert_nos <- which(expert_number[,1]%in%temp_expert_ids)
  par(ps = 12, mar = c(4, 4, 0, 0))
  ...
  legend("topright",
         c(paste0("Question ", que_no),
           colnames(densities)),
           # paste0("Expert ", temp_expert_nos),
           # "Pooled"),
